...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ["ROOT"]
        # buffer在一开始应该存有sentence中的所有元素，按顺序存储
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # 初始化partial_parses，要注意partial_parses是PartialParse()类的实例
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    # shallow copy
    # 浅拷贝会创建一个新对象，但不会递归地复制包含在原对象中的对象。也就是说，浅拷贝仅复制对象的最外层，
    # 而内层的对象仍然是对原始对象中相同对象的引用。
    # 所以后面对partial_parse的处理会直接影响到partial_parses
    unfinished_parses = partial_parses[:]

    # loop
    while(len(unfinished_parses) != 0):
        # take the batch_size parses in unfinished_parses as minibatch
        # 这里也是浅拷贝
        ...
        # perform parse step for each item in minibatch with their transition
        for transition, partial_parse in zip(transitions, minibatch):
            partial_parse.parse_step(transition)

            # remove completed parsers

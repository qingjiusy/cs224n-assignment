...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        @param embeddings (ndarray)：词向量 (num_words,embedding_size)
        @param n_features (int)：输入特征数量
        @param hidden_​​size (int)：隐藏单元数量
        @param n_classes (int)：输出类别数量
        @param dropout_prob (float)：丢失概率
        """
        ...
        ###
        # 注意：可训练变量被声明为“nn.Parameter”，这是一种常用的 API，
        # 用于将张量包含到计算图中以支持更新其梯度。
        # 在这里，我们使用 Xavier 均匀初始化进行权重初始化。
        # 经验表明，这比随机均匀初始化为训练网络提供了更好的初始权重。
        # 有关更多详细信息，请查看这篇精彩的博文：
    ...
    def embedding_lookup(self, w):
        """ Utilize `w` to select embeddings from embedding matrix `self.embeddings`
            ...
            w: 单词索引的输入张量 (batch_size, n_features)
        ...
        # 注意：所有嵌入向量都堆叠并存储为矩阵。模型接收表示单词序列的索引列表，
        # 然后调用此查找函数将索引映射到嵌入序列。此问题旨在测试您对嵌入查找的理解，
        # 因此请勿使用任何高级 API，如 nn.Embedding（我们要求您实现它！）。
        # 注意张量形状并在必要时重塑。在运行代码之前，请确保您知道每个张量的形状！
        ###
        ...
        # for b in range(w.shape[0]): 遍历操作，在batch_size中逐个examples处理
        # torch.index_select(self.embeddings, 0, w[b]) 从 self.embeddings 的第 0 维（即行）中选择索引为 w[b] 的行。
        # self.embeddings 是一个包含所有嵌入的张量
        # w[b] 是 w 的第 b 行，这存有这个feature中的tokens的index
        # 选完以后.view(-1)，将选择的嵌入展平为一维张量
        # .detach().numpy()
        # .detach() 从计算图中分离张量，以确保不会跟踪梯度
        # .numpy() 将张量转换为 NumPy 数组
        # 最后，将这个 NumPy 数组的列表转换为一个新的 PyTorch 张量
